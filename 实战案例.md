...
> 3. 保存初始模型
4. 训练模型 
> 1. 寻找合适的学习率  learn.lr_find()  -> learn.lr_find(suggestions=False) 
...
3. 构造模型 
> 1. 定义learn 选择模型TST(dls.vars, dls.c, dls.len)，损失函数LabelSmoothingCrossEntropyFlat(), 评价指标RocAucBinary(), accuracy
> 2. 寻找合适的学习率
4. 训练模型 
> 1. 训练模型 fit_one_cycle 
