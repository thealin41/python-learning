...
* 对象检测的模型评价标准：IoU，在计算平均精度时，如果两个框的IoU高于某个阈值，就说它们重叠
边界框精度：提供正确的边界框（不要太大或太窄）。
召回率：查找所有目标（不丢失任何目标）。
分类精度：为每个目标输出正确的分类（不要误认为猫是狗）。


本杰明·普朗什,艾略特·安德烈斯. 计算机视觉实战 基于TensorFlow 2（基于TensorFlow 2，全面阐释为目标检测、分割、视频处理、智能手机应用程序等构建高性能系统的实用指南） (智能系统与技术丛书) (Chinese Edition) (Kindle Locations 1902-1903). Kindle Edition. 
##### 实践要点
